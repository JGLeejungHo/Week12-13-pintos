...
static struct file *file_from_fd(int fd) {
  struct thread *t = thread_current();
  struct fd_elem *fe = find_matched_fd(&t->fds, fd);  // fd 리스트에서 fd 번호에 해당하는 엔트리를 찾음
  if (!fe || fe->type != FD_FILE) return NULL;
  return fe->file;  // 매칭된 파일 객체 포인터를 돌려줌
}

void *sys_mmap(void *addr, size_t length, int writable, int fd, off_t offset) {
  // 1. 예외처리
  if (!addr || !is_user_vaddr(addr) || pg_ofs(addr) != 0 || length == 0 || offset < 0 || (offset % PGSIZE) || fd == STDIN_FD || fd == STDOUT_FD) return NULL;

  // 2. fd -> file 객체 얻기
  struct file *file = file_from_fd(fd);
  if (!file) return NULL;                   // FD가 유효X → 실패
  if (file_length(file) <= 0) return NULL;  // 파일 길이 0(또는 음수) → 실패

  // 3. 매핑 전용 핸들
  struct file *fm = file_reopen(file);
  if (!fm) return NULL;

  /* 4. 실제 매핑: do_mmap이 lazy 등록/소유권 가짐 */
  void *mapped = do_mmap(addr, length, writable, fm, offset);

  if (!mapped) file_close(fm); /* 실패: 핸들 반납 */
  return mapped;               /* 성공: 시작 VA, 실패: NULL */
}

void sys_munmap(void *addr) {
  if (!addr || !is_user_vaddr(addr)) return;  // 주소 유효성만 검증
  do_munmap(addr);
}
